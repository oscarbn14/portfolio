...
Dashboard: https://public.tableau.com/app/profile/oscar.batiz/viz/MarketingCampaign_17302317607010/Dashboard1
...
FROM marketing_campaign
GROUP BY cust_age, marital_status
ORDER BY cust_age, marital_status;
/* 
...
SELECT 
	...
	END AS cust_age,
	marital_status,
    SUM(spent_wine) AS wine_perc,
    SUM(spent_fruits) AS fruits_perc,
    SUM(spent_meat) AS meat_perc,
	SUM(spent_fish) AS fish_perc,
    SUM(spent_sweets) AS sweets_perc,
    SUM(spent_gold) AS gold_perc
FROM marketing_campaign
GROUP BY cust_age, marital_status
ORDER BY cust_age, marital_status;
/* 
	In this query, we can observe the spending patterns of each customer type 
